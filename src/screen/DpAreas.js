...
const DpAreas = () => {
 ...
 <Breadcrumb />
      ...
      >
        ...
        {areas?.map((area) => (
          <div key={area.id} className="flex flex-col border rounded-xl font-roboto lg:min-w-[21rem]">
            <div className="flex flex-row bg-gray-200 p-[8px] rounded-xl gap-[5%]">
              <img src={area?.thumbnail_image} alt={area?.dp_image} className={`w-[50%] min-w-[10rem] h-40 rounded-lg ${area?.thumbnail_image ? '' : 'bg-gray-600'}`}/>
              <div className="flex flex-row p-1 justify-between w-[50%] min-w-[10rem]">
