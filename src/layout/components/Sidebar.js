  ...
  return (
    <div className={`h-[100%] fixed ${!showSlide ? 'slide-out-in ' : 'slide-in'} py-8 w-fit bg-[#D9E2FF] flex flex-col justify-between`}>
      <div className='flex flex-col items-center'>
        ...
        <img src="/assets/inframent-logo.png" alt="logo image" className={`bg-white cursor-pointer rounded-full drop-shadow-lg w-[80%] max-w-[120px]`}/>
        </Link>
        <div className='w-[80%] mt-[3rem] flex flex-col items-center gap-3'>
            {
            navLinks.map((nav) => (
                <Link to={nav.link} key={nav.id} className={`flex flex-row  items-center ${active === nav.id ? 'bg-[#355CA8] text-white' : ''}  rounded-full ${!showSlide ? 'h-[4rem] w-[4rem] justify-center items-center' : 'w-[100%] h-[50px] pl-4 gap-2 drop-shadow-xl'}`}>
